...
stopHFA = function(hfa, targets)
	if (not(hfa.currentBehavior == nil) and
		-- ... for some reason ...
		not(hfa.currentBehavior == start) and
		not(hfa.currentBehavior.stop == nil) and
		not(hfa.interruptable)) then
		hfa.currentBehavior.stop(hfa.currentBehavior, hfa.behaviorTargets)
	end
end
...
goHFA = function(hfa, targets)
		...
		if (not(newBehavior == hfa.currentBehavior)) then
			if ((not(hfa.currentBehavior == nil)) and
				(not(hfa.currentBehavior.stop == nil))) then
					hfa.currentBehavior.stop(hfa.currentBehavior, oldbehaviorTargets)
			end
	...
	if (hfa.currentBehavior == null) then
		print("WARNING (goHFA): NULL current behavior")
	elseif (not(hfa.currentBehavior.go == null)) then
		hfa.goReturnValue = hfa.currentBehavior.go(hfa.currentBehavior, hfa.behaviorTargets)
	end
end
...
makeHFA = function(name, transition, interruptable)
	return { ["goReturnValue"] = nil, ["name"] = name, 
			 ["start"] = startHFA, ["stop"] = stopHFA, ["go"] = goHFA, 
		 	 ...
		 	 ["parent"] = nil, ["counter"] = 0, ["timer"] = 0, 
		 	 ["done"] = false, ["failed"] = false, ["current"] = nil,
			 ["propagateFlags"] = false, ["targets"] = nil, 
			 ["behaviorTargets"] = nil }
end
