...
Easysocket::Easysocket(string host, int port, int type)
{
	server_type = type;
	global_server_defined = 1;
	global_server_port = port;
	global_server_addr = host;
}
...
int Easysocket::readn(char* ptr, int nbytes)
{
	int nleft, nread;
	nleft = nbytes;
	while (nleft > 0) {
...
int Easysocket::writen(char* ptr, int nbytes)
...
int Easysocket::readline(char* ptr, int maxlen)
...
int Easysocket::connect_to_socket()
...

{
	...
	if (!global_server_defined) {
		...
		global_server_addr = SERV_HOST_ADDR;
	}
	...
	serv_addr.sin_addr.s_addr = inet_addr(global_server_addr.c_str());
