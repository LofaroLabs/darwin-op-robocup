 ...
 * @author drew
 */
public class GotoWhileFacingGoalie extends Behavior {
    private static final long serialVersionUID = 1L;
    private boolean started = false;
    
    public GotoWhileFacingGoalie() 
    	{
        name = "GotoWhileFacingGoalie";
        }
    
    public GotoWhileFacingGoalie(Target where, Target facing) 
    	{
    	this();
        }
...
*/
    ...
    public void start(Agent agent, Macro parent, Horde horde) {
        ...
        ((DarwinAgent) agent).sendMotion(Motions.gotoWhileFacingGoalie(gotoPose, facing));
        started = true;
    }
    ...
    public void go(Agent agent, Macro parent, Horde horde) {
        ...
        if (!started) {
            ...
            Double2D facing = new Double2D(dp.getClosestToBallLocX(), dp.getClosestToBallLocY());
            Double2D gotoPose = new Double2D(dp.getPenaltyBoundsX(), (facing.y < dp.getPenaltyBoundsY()) ? facing.y : dp.getPenaltyBoundsY());
            ((DarwinAgent) agent).sendMotion(Motions.gotoWhileFacingGoalieUpdate(gotoPose, facing));
        }
    }
