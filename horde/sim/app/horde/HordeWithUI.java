...
public abstract class HordeWithUI extends GUIState implements MacroObserver
    ...
    public JPopupMenu createTargetMenu(final TrainableMacro macro, final Targeting targeting)
        ...
        for(int i = 0; i < arguments.length; i++) 
    		...
    		// Targets can be null: this happens when we save out a behavior, and null out some
    		// of its unused targets, but we don't compress the array because the underlying wrappers
    		// all refer to the array by index and we don't want to mess with them.  Then when the
    		// behavior is loaded in again, some of the targets are now null because they're unused,
    		// which is just fine.  So this code is WRONG:
            //else System.err.println("NULL TARGET " + i + " " + targeting + " " + argumentNames[i] + " this should not happen!");
