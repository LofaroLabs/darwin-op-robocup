...
import sim.app.horde.scenarios.pioneer.features.BlobXCoord;
...
public class RobotPanel extends JPanel implements MacroObserver
            ...
            for (int i = 0; i < robots.size(); i++)
                ...
                if (robots.get(i) == src)
                    ...
                    for (int j = 0; j < horde.agents.size(); j++)
                        {
                        PioneerAgent a = horde.agents.get(j);
    ...
    public RobotPanel(final JFrame mainWindow) {
        ...
        final BlobXCoord seeGreen = new BlobXCoord();
        seeGreen.setTarget(0, new AttackerColor()); 
        final BlobXCoord seeBlue = new BlobXCoord();
        seeBlue.setTarget(0, new ScatterColor()); 

        final long SCATTER_TIME = 10000; 
                
        final long[] lastSeenBlue = new long[] { System.currentTimeMillis() - SCATTER_TIME - 1 };
            ...
            public void run()
                ...
                SwingUtilities.invokeLater(new Runnable()
                    ...
                    public void run()
                        ...
                        //send info to GodAgent 
                        double seeAttack = 0, oneDone = 0, allDone = 0, seeScatter = 0;
                        int doneCnt=0;
                        for (int i=0; i < horde.agents.size(); i++) { 
                            PioneerAgent a = horde.agents.get(i); 
                            if (seeGreen.getValue(a, (Macro)a.getBehavior(), horde) >= 0)
                                seeAttack = 1; 
                                                        
                            if (((TrainableMacro)a.getBehavior()).getDone()) 
                                doneCnt++; 
                                                
                            if (seeBlue.getValue(a, (Macro)a.getBehavior(), horde) >= 0) 
                                {
                                lastSeenBlue[0] = System.currentTimeMillis();
                                seeScatter = 1;
                                }
                            else  // it's under 5 seconds from last time
                                {
                                if (System.currentTimeMillis() - lastSeenBlue[0] < SCATTER_TIME)  
                                    seeScatter = 1; 
                                }
                            }
                        ...
                        if (doneCnt > 0) 
                            oneDone = 1; 
                        if (doneCnt == horde.agents.size())
                            allDone = 1; 
                                                
                                                
                        //receive any directives from GodAgent
                                                
                        ControlledMacro.toSocket.println(seeAttack); 
                        ControlledMacro.toSocket.println(oneDone);
                        ControlledMacro.toSocket.println(allDone);
                        ControlledMacro.toSocket.println(seeScatter);
        ...
        // add a panel for the sonar display
        sonarRenderer = new SonarRenderer(robotThread);
        box.add(sonarRenderer, BorderLayout.WEST);

        // add a simple display for the camera
        cameraRenderer = new CameraRenderer(robotThread);
        box.add(cameraRenderer, BorderLayout.EAST);
        ...
        saveTreesButton.addActionListener(new ActionListener()
            ...
            public void actionPerformed(ActionEvent e)
                ...
                if (fileName != null)
                    ...
                    for (int i = 0; i < s.length; i++)
                        ...
                        if (s[i].equalsIgnoreCase(fileName))
                            ...
                            if (JOptionPane.showConfirmDialog(null, "The name you have chosen (" + fileName
                                    + ") already exists.  Do you wish to overwrite it?", "Overwrite Existing Macro?",
                    ...
                    // save it!!!
                    horde.save(fileName, KeyStroke.getKeyStroke('s'));
                    }
    ...
    /**
     * Called to change the behaviors to those in the given Macro. Updates all
     * the buttons, adds new menus and listeners to them, and updates the
        ...
        if (m != null)
            ...
            for (int x = 0; x < m.behaviors.length; x++)
                ...
                if (m.behaviors[x].getKeyStroke() != null)
                    button.registerKeyboardAction(listener, m.behaviors[x].getKeyStroke(),
     ...
     * We then update the current hilighted behavior button as a result. A
    ...
    /**
     * Hilights the given button and de-hlights the others. Sets the current
     * hilightedButton to this button.
     */
                    ...
                    jmi.addActionListener(new ActionListener()
                        ...
                        public void actionPerformed(ActionEvent e)
                            ...
                            if (targets[_j] instanceof Parameter)
                                {
                                Wrapper w = new Wrapper(targeting.getTargetName(_i), ((Parameter) targets[_j])
                                    .getIndex());
    ...
    public void buildAddList()
        ...
        final Feature[] f = Feature.provideAllFeatures(horde.getTrainingAgent());
