...
{
public:
	Easysocket(string, int, int);
	~Easysocket();

	int readn(char* ptr, int nbytes); /*  Read N Chars, Else <0 */
	int writen(char* ptr, int nbytes); /*  Write N Chars, Else <0 */
	int readline(char* ptr, int maxlen); /*  Read A Line, Else <0 */
...
private:
	int fd;
	int global_server_defined; /*  We've Got A Non-Default Port and non-default server address */
	int global_server_port; /*  The Actual TCP Port */
	string global_server_addr; /*  the actual tcp address */
