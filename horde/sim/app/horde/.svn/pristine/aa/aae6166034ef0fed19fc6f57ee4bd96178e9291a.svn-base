 ...
 * 
 * <h2>Overview</h2> This class contains the communications layer-independent parts of how to communicate with a Roomba. It does assume a very serial
 * port-like interaction.
 * 
 ...
 * 
 * 
 * @author Tod E. Kurt SVN id value is $Id: RoombaComm.java 225 2011-10-26 21:06:15Z bouchier $
 */
public class Create implements Runnable
    ...
    public boolean connect(String portid)
        ...
        if (s.length < 2)
            ...
            try
                ...
                thread.start();
                // this.setConnected(this.updateSensors()); // test if we are
                // connected or not
    ...
    public void disconnect()
        {
        System.err.print("Closing Create streams..."); 
        try
            {
        ...
        System.err.print("Closing Create socket..."); 
        try
            {
            if (socket != null)
                socket.close();
        ...
        while (Thread.currentThread() == thread)
            ...
            try
                ...
                while (input != null)
                    ...
                    buffer[bufferLast++] = (byte) input.read(); // this will
                    // block - wait
                    // until there's
                    // something
                    // logmsg("TCP received sensor data: " +
                    // buffer[bufferLast-1]);
    ...
    public int readBytesUntil(int interesting, byte outgoing[])
        ...
        synchronized (getBuffer())
            ...
            if (length > outgoing.length)
                {
                System.err.println("readBytesUntil() byte buffer is" + " too small for the " + length + " bytes up to and including char "
                    + interesting);
                return -1;
                }
            // byte outgoing[] = new byte[length];
    ...
    /**
     * Some "virtual" serial ports like Bluetooth serial on Windows return weird errors deep inside RXTX if an opened port is used before the virtual
     * COM port is ready. One way to check that it is ready is to look for the DSR line going high. However, most simple, real serial ports do not do
     * hardware handshaking so never set DSR high. Thus, if using Bluetooth serial on Windows, do: roombacomm.waitForDSR = true; before using it and
     * see if it works.
     */
    ...
    // public Create(RobotConnection rc) {
    // robotConnection = rc;
    // }
    public void startAutoUpdate(int updateTime)
        {
    ...
    /**
     * Query Roomba for sensor status and sync its state with this object's Subclasses should query Roomba and fill up 'sensor_bytes' with the full
     * sensor data set If a RooombaComm object is constructed with 'autoUpdate' true, calling this method is not required because a separate thread is
    ...
    /**
     * Wake's Roomba up, if possible, thus optional To wake up the Roomba requires twiddling its DD line, often hooked up to the RS-232 DTR line,
     * which may not be available in some implementations
     */
    public void wakeup()
        {
        logmsg("subclass has not implemented");
        // byte cmd[] = { (byte)POWER, (byte)v, (byte)power_color,
    ...
    /**
     * Put Roomba in safe mode. As opposed to full mode. Safe mode is the preferred working state when playing with the Roomba as it provides some
     * measure of autonomous self-preservation if it encounters a cliff or is picked up If that happens it goes into passive mode and must be
     * 'reset()'.
     * 
     * @see #reset()
     */
    ...
    /**
     * Reset Roomba after a fault. This takes it out of whatever mode it was in and puts it into safe mode. This command also syncs the object's
     * sensor state with the Roomba's by calling updateSensors()
     * 
     * @see #startup()
     * @see #updateSensors()
     */
     ...
     * 
     * @param packetcode
     *            one of SENSORS_ALL, SENSORS_PHYSICAL, SENSORS_INTERNAL, or SENSORS_POWER, or for roomba 5xx, it is the sensor packet number (from
     *            the spec)
     */
    ...
    public byte[] sensors()
        ...
        return copySensorBytes();
        }

    private byte[] copySensorBytes()
        {
        byte[] sbytes = new byte[sensor_bytes.length];
        System.arraycopy(sensor_bytes, 0, sbytes, 0, sbytes.length);
        return sbytes;
        }

    /**
     * Read roomba 26-byte sensor record using robotConnection. Tries once to read valid data, allowing 100ms timeout on each attempt.
     * 
    ...
    // default:
    // System.err.println("Invalid sensor group in updateSensors(): " +
    // sensorGroup);
    // return false;
    // }
    // sensors(sensorGroup);
    ...
    // }
    //
    // // readBot either read requested # of bytes or returned a null
    // (indicating timeout.
    ...
    /**
     * Go straight at the current speed for a specified distance. Positive distance moves forward, negative distance moves backward. This method
     ...
     * 
     * @param speed
     *            to spin at, positive to spin left, negative to spin right
     */
    ...
    /**
     * Move the Roomba via the low-level velocity + radius method. See the 'Drive' section of the Roomba ROI spec for more details. Low-level command.
     * 
     * @param velocity
    ...
    /**
     * Play a musical note Does it via the hacky method of defining a one-note song & playing it Uses up song slot 15. If another note is played
    ...
    public void playNote(int note, int duration)
        ...
        byte cmd[] = { (byte) SONG, 3, 1, (byte) note, (byte) duration, // define song
     ...
     */
    public void setLEDs(boolean status_green, boolean status_red, boolean spot, boolean clean, boolean max, boolean dirt, int power_color,
        int power_intensity)
        {
    ...
    // 500 series
    public void setLEDsOI(boolean checkRobot, boolean spot, boolean dock, boolean dirt, int power_color, int power_intensity)
        {
        updateDisplay("setLEDsOI (" + checkRobot + ")(" + spot + ")(" + dock + ")(" + dirt + ")(" + power_color + ")(" + power_intensity + ")",
            this.debug);
    ...
    public boolean computeSafetyFault()
        {
        safetyFault = (sensor_bytes[BUMPSWHEELDROPS] & WHEELDROP_MASK) != 0 || sensor_bytes[CLIFFLEFT] == 1 || sensor_bytes[CLIFFFRONTLEFT] == 1
     ...
     */
    // * this likely needs to know about protocal to know how to read the
    // sensors */
    public String sensorsAsString()
        ...
        return "*****\n" + "bump:" + (bumpLeft(sData) ? "l" : "_") + (bumpRight(sData) ? "r" : "_") + " wheel:" + (wheelDropLeft() ? "l" : "_")
            + (wheelDropCenter() ? "c" : "_") + (wheelDropRight() ? "r" : "_") + " wall:" + (wall(sData) ? "Y" : "n") + " cliff:"
            + (cliffLeft() ? "l" : "_") + (cliffFrontLeft() ? "L" : "_") + (cliffFrontRight() ? "R" : "_") + (cliffRight() ? "r" : "_")
            + " dirtL:" + dirtLeft() + " dirtR:" + dirtRight() + "\n" + "vwal:" + virtual_wall() + " motr:" + motor_overcurrents() + " dirt:"
            + dirt_left() + "," + dirt_right() + " remo:" + hex(remote_opcode()) + " butt:" + hex(buttons()) + " dist:" + distance() + " angl:"
            + angle(sData) + "\n" + "chst:" + charging_state() + " volt:" + voltage() + " curr:" + current() + " temp:" + temperatureF() + "F"
            + " chrg:" + charge() + " capa:" + capacity() + sd;
        }
    ...
    public String chargeDataAsString()
        ...
        return "Charging State: " + charging_state() + " Temperature: " + temperatureF() + "F\n" + "Voltage:        " + voltage() + " Current:     "
            + current() + "\n" + "Capacity:       " + capacity() + "  Charge:      " + charge() + sd;
        }
    ...
    /** Left bump sensor */
    public boolean bumpLeft(byte[] sData)
        {
        if (sData != null)
            return (sData[BUMPSWHEELDROPS] & BUMPLEFT_MASK) != 0;

        return (sensor_bytes[BUMPSWHEELDROPS] & BUMPLEFT_MASK) != 0;
        }

    /** Right bump sensor */
    public boolean bumpRight(byte[] sData)
        {
        if (sData != null)
            return (sData[BUMPSWHEELDROPS] & BUMPRIGHT_MASK) != 0;
    ...
    /** Can we see a wall? */
    public boolean wall(byte[] sData)
        {
        if (sData != null)
            return sData[WALL] != 0;
     ...
     */
    public short angle(byte[] sData)
        {

        if (sData != null)
            return toShort(sData[ANGLE_HI], sData[ANGLE_LO]);
        else
    ...
    public float angleInDegrees(byte[] sData)
        {
        return (float) angle(sData) / millimetersPerDegree;
        }
    ...
    public float angleInRadians(byte[] sData)
        {
        return (float) angle(sData) / millimetersPerRadian;
        }
    ...
    protected void readConfigFile()
        ...
        try
            ...
            //logmsg("read config port: " + serialPort + " protocol: "
            //              + getProtocol() + " waitDSR: " + waitForDSR);
            } catch (IOException e)
            {
    ...
    public void setLEDs(Create roombacomm)
        {
        ...
        if (this.protocol.equalsIgnoreCase("SCI"))
            {
            roombacomm.setLEDs(this.greenOn, this.redOn, this.toggleSpot, this.toggleClean, this.toggleMax, this.toggleDirt, this.power_color,
                this.power_int);
            }
        if (this.protocol.equalsIgnoreCase("OI"))
            ...
            updateDisplay("Checkrobot(" + this.toggleCheckRobot + "),Spot(" + this.toggleSpot + "),Dock(" + this.toggleDock + "),Dirt("
                + this.toggleDirt + "),Pcolor(" + this.power_color + "),Pint(" + this.power_int + ")", true);
            }
        }

    public void setChgGreenLED(Create roombacomm, boolean green)
        {
        this.greenOn = green;
    ...
    public void setChgRedLED(Create roombacomm, boolean red)
        {
        this.redOn = red;
    ...
    public void setChgSpotLED(Create roombacomm, boolean spot)
        {
    ...
    public void setChgCleanLED(Create roombacomm, boolean clean)
        {
    ...
    public void setChgMaxLED(Create roombacomm, boolean max)
        {
    ...
    public void setChgDirtLED(Create roombacomm, boolean dirt)
        {
    ...
    public void setChgPowerColorLED(Create roombacomm, int power_color)
        {
    ...
    public void setChgPowerIntensityLED(Create roombacomm, int power_intensity)
        {
    ...
    public void setChgCheckRobotLED(Create roombacomm, boolean toggleCheckRobot)
        {
        this.toggleCheckRobot = toggleCheckRobot;
        this.setLEDs(roombacomm);
        }

    public void setChgDockLED(Create roombacomm, boolean toggleDock)
        {
    ...
    /**
     * Returns the number of bytes that have been read from serial and are waiting to be dealt with by the user. (from processing.serial.Serial)
     * 
     * private int available() { return (bufferLast - bufferIndex); }
     * 
     * /** Return a byte array of anything that's in the serial buffer. Not particularly memory/speed efficient, because it creates a byte array on
     * each read, but it's easier to use than readBytes(byte b[]) (see below). (from processing.serial.Serial)
     * 
     * private byte[] readBytes() { if (bufferIndex == bufferLast) return null;
     * 
     * synchronized (buffer) { int length = bufferLast - bufferIndex; byte outgoing[] = new byte[length]; System.arraycopy(buffer, bufferIndex,
     * outgoing, 0, length);
     * 
     ...
     * 
     * public int readBytes(byte outgoing[]) { if (bufferIndex == bufferLast) return 0;
     * 
     * synchronized (buffer) { int length = bufferLast - bufferIndex; if (length > outgoing.length) length = outgoing.length; System.arraycopy(buffer,
