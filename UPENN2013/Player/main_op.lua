...
function update()
  ...
  if waiting>0 then --Waiting mode, check role change
    ...
    if cur_role==0 then
      ...
      gcm.set_team_role(0);--actual goalie?
      ...
      gcm.set_team_role(1); -- actual player?
    ...
    --gcm.set_team_role(1);--always be an attacker
...
if (webots) then
   print("@##@@## you should only see this if you are using webots")  
require('cognition');
  cognition.entry();
